...
const chartColours = ['#2563eb','#0f9f8f','#16a34a','#d97706','#7c3aed','#e11d48','#0891b2','#65a30d'];
...
function periodValues() {
  ...
  const p = payResult;
  const yearly = { gross:p.gross,tax:p.tax,ni:p.ni,net:p.net,expenses:p.expensesYear,remaining:p.remainingYear,label:'Take-home per year',digits:0 };
...
function renderPeriod() {
  ...
  $('periodRemaining').textContent = `Money remaining after expenses: ${money(values.remaining, values.digits)}`;
...
function renderEstimate() {
  ...
  const content = {
    ...
    ], p.paidLeave ? 'Paid leave keeps normal contractual pay while reducing actual worked hours.' : 'Unpaid leave is estimated as normal weekly gross pay multiplied by unpaid leave weeks. Real payroll methods can differ by contract.'),
    ...
    ], 'Income Tax is an annual estimate. NI uses the selected weekly or monthly payroll pattern.'),
    ...
    ], 'Money remaining after expenses is not payslip take-home pay. It is take-home minus the costs you entered.'),
...
function renderDonut(donutId, centreId, legendId, slices, centreText, extraRows = []) {
  const donut = $(donutId);
  const legend = $(legendId);
  const centre = $(centreId);
  const usable = slices.map((slice, index) => ({
    ...slice,
    value: Math.max(0, Number(slice.value) || 0),
    colour: slice.colour || chartColours[index % chartColours.length]
  })).filter(slice => slice.value > 0);
  const total = usable.reduce((sum, slice) => sum + slice.value, 0);
  centre.textContent = centreText;
  if (!total) {
    donut.style.background = '#e7edf4';
    legend.innerHTML = '<div><i style="background:#cbd5e1"></i><span>No figures entered yet</span><strong>—</strong></div>';
    return;
  }
  let current = 0;
  const segments = usable.map(slice => {
    ...
    return `${slice.colour} ${start}% ${current}%`;
  });
  donut.style.background = `conic-gradient(${segments.join(',')})`;
  const rows = [...usable, ...extraRows].map((slice, index) => ({
    ...slice,
    colour: slice.colour || chartColours[(usable.length + index) % chartColours.length]
  }));
  legend.innerHTML = rows.map(slice => `<div><i style="background:${slice.colour}"></i><span>${slice.label}</span><strong>${money(slice.value)}</strong></div>`).join('');
}

function renderDonuts() {
  if (!payResult || !projectionResult) return;
  const p = payResult;
  const expenseShown = Math.min(Math.max(0, p.expensesYear), Math.max(0, p.net));
  const remainingShown = Math.max(0, p.remainingYear);
  const overspend = Math.max(0, p.expensesYear - p.net);
  renderDonut('moneyDonut','moneyDonutCentre','moneyLegend',[
    {label:'Income Tax',value:p.tax,colour:'#2563eb'},
    {label:'Employee NI',value:p.ni,colour:'#0f9f8f'},
    {label:'Expenses',value:expenseShown,colour:'#d97706'},
    {label:'Money remaining',value:remainingShown,colour:'#16a34a'}
  ], pct(p.gross > 0 ? remainingShown / p.gross * 100 : 0), overspend > 0 ? [{label:'Overspend beyond take-home',value:overspend,colour:'#dc2626'}] : []);

  const expenseLabels = {
    housing:'Housing',councilTax:'Council Tax',utilities:'Utilities & insurance',food:'Food',
    transport:'Transport',living:'Phone, childcare & care',debt:'Debt',finance:'Finance',
    subscriptions:'Subscriptions',optional:'Flexible spending',other:'Other'
  };
  renderDonut('spendingDonut','spendingDonutCentre','spendingLegend',
    Object.entries(p.expenses).map(([key,value],index)=>({label:expenseLabels[key]||key,value,colour:chartColours[index%chartColours.length]})),
    money(p.expensesMonth)
  );

  const pension = projectionResult.pension;
  const netGrowth = Math.max(0, pension.investmentGrowth - pension.fees);
  renderDonut('pensionDonut','pensionDonutCentre','pensionLegend',[
    {label:'Current balance',value:num('currentPension'),colour:'#2563eb'},
    {label:'Your deposits',value:pension.employeeDeposits,colour:'#0f9f8f'},
    {label:'Employer deposits',value:pension.employerDeposits,colour:'#d97706'},
    {label:'Net estimated growth',value:netGrowth,colour:'#16a34a'}
  ], money(pension.balanceAtRetirement), pension.fees > 0 ? [{label:'Modelled fees',value:pension.fees,colour:'#dc2626'}] : []);
}
...
function lineChart(container, labels, series) {
  const width = 720, height = 300, pad = {l:54,r:18,t:25,b:40};
  ...
  let svg = `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Financial projection line chart"><defs>`;
  series.forEach((item,index)=>{
    const colour=item.colour||chartColours[index%chartColours.length];
    svg+=`<linearGradient id="area-${container.id}-${index}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${colour}" stop-opacity=".22"/><stop offset="100%" stop-color="${colour}" stop-opacity="0"/></linearGradient>`;
  });
  svg+='</defs>';
  for(let i=0;i<5;i++){
    const yy=pad.t+i*(height-pad.t-pad.b)/4;
    const val=maxValue-i*range/4;
    svg+=`<line class="chart-gridline" x1="${pad.l}" y1="${yy}" x2="${width-pad.r}" y2="${yy}"/><text class="chart-label" x="4" y="${yy+4}">${compactMoney(val)}</text>`;
  }
  svg+=`<line class="chart-axis" x1="${pad.l}" y1="${height-pad.b}" x2="${width-pad.r}" y2="${height-pad.b}"/>`;
  labels.forEach((label,i)=>{
    if(labels.length<=10||i===0||i===labels.length-1||i%Math.ceil(labels.length/8)===0){
      svg+=`<text class="chart-label" text-anchor="middle" x="${x(i)}" y="${height-13}">${label}</text>`;
    }
  });
  series.forEach((item,index)=>{
    const colour=item.colour||chartColours[index%chartColours.length];
    ...
    if(points.length>1){
      const areaPath=`${linePath} L ${points.at(-1)[0]} ${height-pad.b} L ${points[0][0]} ${height-pad.b} Z`;
      svg+=`<path class="chart-area" fill="url(#area-${container.id}-${index})" d="${areaPath}"/>`;
    }
    svg+=`<path class="chart-line" stroke="${colour}" d="${linePath}"/>`;
    points.forEach((point,i)=>{
      svg+=`<circle class="chart-point" fill="${colour}" cx="${point[0]}" cy="${point[1]}" r="4"><title>${item.name}, ${labels[i]}: ${money(item.values[i])}</title></circle>`;
    });
  });
  svg+='</svg>';
  const legend=`<div class="chart-legend">${series.map((item,i)=>`<span><i style="background:${item.colour||chartColours[i%chartColours.length]}"></i>${item.name}</span>`).join('')}</div>`;
...
function changeText(start, end) {
  if (!Number.isFinite(start) || start === 0) return { text:'Starting estimate', direction:'' };
  const change=(end-start)/Math.abs(start)*100;
  return {text:`${change>=0?'+':''}${change.toFixed(1)}% from start`,direction:change>0?'up':change<0?'down':''};
}

function renderProjectionSummary(rows) {
  const start=rows[0], end=rows.at(-1);
  if(!start||!end)return;
  const incomeChange=changeText(start.net,end.net);
  const pensionStart=Math.max(num('currentPension'),start.pensionBalance);
  const pensionChange=changeText(pensionStart,end.pensionBalance);
  $('projectionStartNet').textContent=money(chartMoney==='real'?start.realNet:start.net);
  $('projectionEndNet').textContent=money(chartMoney==='real'?end.realNet:end.net);
  $('projectionEndPension').textContent=money(chartMoney==='real'?end.realPensionBalance:end.pensionBalance);
  $('projectionEndRemaining').textContent=money(chartMoney==='real'?end.realRemaining:end.remaining);
  $('projectionIncomeChange').textContent=incomeChange.text;
  $('projectionIncomeChange').className=incomeChange.direction;
  $('projectionPensionChange').textContent=pensionChange.text;
  $('projectionPensionChange').className=pensionChange.direction;
}
...
function renderCharts() {
  ...
  const grossValues=seriesValues(rows,'gross','realGross');
  const netValues=seriesValues(rows,'net','realNet');
  const remainingValues=seriesValues(rows,'remaining','realRemaining');
  lineChart($('incomeChart'),labels,[
    {name:'Gross pay',values:grossValues,colour:'#2563eb'},
    {name:'Take-home',values:netValues,colour:'#0f9f8f'},
    {name:'Money remaining',values:remainingValues,colour:'#16a34a'}
  ]);
  let employeeTotal=0, employerTotal=0;
  const employeeValues=rows.map(row=>chartMode==='cumulative'?(employeeTotal+=row.employeePension):row.employeePension);
  const employerValues=rows.map(row=>chartMode==='cumulative'?(employerTotal+=row.employerPension):row.employerPension);
  ...
  lineChart($('pensionChart'),labels,[
    ...
    {name:chartMode==='cumulative'?'Your deposits total':'Your contribution',values:employeeValues,colour:'#0f9f8f'},
    {name:chartMode==='cumulative'?'Employer deposits total':'Employer contribution',values:employerValues,colour:'#d97706'}
  ]);
  const end=rows.at(-1);
  $('incomeChartHeadline').textContent=money(chartMode==='cumulative'?netValues.at(-1):(chartMoney==='real'?end.realNet:end.net));
  $('pensionChartHeadline').textContent=money(chartMoney==='real'?end.realPensionBalance:end.pensionBalance);
  renderProjectionSummary(rows);
}
...
function renderBudget() {
  ...
  const items=[
    ['Monthly take-home',money(p.netMonthly)],['Entered monthly costs',money(p.expensesMonth)],['Money remaining',money(p.remainingMonth)],
  ...
  if(housingPct>35)tips.push(`Housing uses about ${housingPct.toFixed(0)}% of take-home, making it one of the strongest pressures on the budget.`);
  ...
  if(p.gross>0&&p.overtimeGross/p.gross>.2)tips.push('More than 20% of gross income comes from overtime. Avoid making essential commitments that require overtime to continue.');
...
function renderCar() {
  ...
  $('carResults').innerHTML=`<div class="dashboard-head"><div><strong>Car affordability result</strong><p>This rating uses take-home, entered expenses and the full car cost.</p></div><span class="rating ${r.status.code}">${r.status.label}</span></div><div class="dashboard-grid">${[
  ...
  ].map(([a,b])=>`<div><span>${a}</span><strong>${b}</strong></div>`).join('')}</div><p class="clarification">This does not predict finance approval. Lenders consider credit history, income stability, existing borrowing and their own rules. Remove an old car cost from monthly spending when this estimate replaces it.</p>`;
}
...
function resetAll() {
  ...
  $('resultStatus').textContent='Enter your figures and press “Update all calculations”.';
  ['periodNet','metricGross','metricTax','metricNi','metricNet','metricExpenses','metricRemaining','effectiveDeduction','projectionStartNet','projectionEndNet','projectionEndPension','projectionEndRemaining','incomeChartHeadline','pensionChartHeadline'].forEach(id=>{if($(id))$(id).textContent='—';});
  $('periodRemaining').textContent='Money remaining after expenses: —';
  ...
  ['moneyDonut','spendingDonut','pensionDonut'].forEach(id=>$(id).style.background='#e7edf4');
  $('moneyDonutCentre').textContent='—';$('spendingDonutCentre').textContent='—';$('pensionDonutCentre').textContent='—';
  ['moneyLegend','spendingLegend','pensionLegend'].forEach(id=>$(id).innerHTML='<div><i style="background:#cbd5e1"></i><span>Calculate to view</span><strong>—</strong></div>');
...
document.querySelectorAll('.estimate-tab').forEach(button=>button.addEventListener('click',()=>{
  activeEstimate=button.dataset.estimate;
  document.querySelectorAll('.estimate-tab').forEach(item=>item.classList.toggle('active',item===button));
  renderEstimate();
}));
document.querySelectorAll('.chart-range').forEach(button=>button.addEventListener('click',()=>{
  chartYears=button.dataset.years;
  document.querySelectorAll('.chart-range').forEach(item=>item.classList.toggle('active',item===button));
  renderCharts();
}));
...
document.querySelectorAll('.chart-money').forEach(button=>button.addEventListener('click',()=>{
  chartMoney=button.dataset.money;
  document.querySelectorAll('.chart-money').forEach(item=>item.classList.toggle('active',item===button));
  renderCharts();
}));
...
inputIds.forEach(id=>$(id)?.addEventListener('input',()=>{
  ...
  if(expenseIds.includes(id))updateExpensePreview();
  if(payResult)$('resultStatus').textContent='Inputs changed. Press Update calculation to apply them.';
}));
